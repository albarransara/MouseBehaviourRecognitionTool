...
def make_model(input_shape, layers, dropout_rate=0.5, num_layers=1, num_units=128, kernel_size=3, norm='batch', loss='binary_crossentropy', optimizer='adam', learning_rate=0.001, num_heads=6, b_idx='Groomig',name=None):
    """
    ...
    if layers == 'lstm':
        ...
        model.add(keras.layers.Bidirectional(keras.layers.LSTM(num_units, return_sequences=True)))
        for i in range(num_layers - 1):
            num_units = num_units // 2
            model.add(keras.layers.Bidirectional(keras.layers.LSTM(num_units, return_sequences=True)))
    elif layers == 'tcn':
        ...
        for i in range(num_layers - 1):
            num_units = num_units // 2
            model.add(TCN(num_units, kernel_size=kernel_size, use_batch_norm=batch, use_layer_norm=layer, use_weight_norm=weight, return_sequences=True))
    elif layers  == 'transformer':
        model = get_model(input_shape, input_shape[0], num_heads=num_heads)
        #if BEHAVIOR_NAMES[b_idx] == 'Grooming':
        #    model = keras.models.load_model(f'{name}_grooming.keras')
        #elif BEHAVIOR_NAMES[b_idx] == 'Rearing':
        #    model = keras.models.load_model(f'{name}_rearing.keras')
        model.compile(
            optimizer=optimizer,
            loss=loss,
            metrics=METRICS
...
def train_test_model(config):
    ...
    # Split data into sequences
    X_train, y_train = generate_sequences(dataset['train'], config['sequence_length'], config['norm'])
    X_test, y_test = generate_sequences(dataset['test'], config['sequence_length'],config['norm'])
    ...
    for b_idx in range(y_train.shape[2]):
        ...
        model = make_model(
            ...
            config['num_layers'], config['num_units'],
            loss=config['loss'], optimizer=config['optimizer'],
            learning_rate=config['learning_rate'], num_heads=config['num_heads'], b_idx=b_idx, name=config['model_name']
        )

        # Define the callback to save the best weights for the 3 models
        if config['layers'] == 'tcn':
            model_name = config['model_name']
            if BEHAVIOR_NAMES[b_idx] == 'Grooming':
                sav = ModelCheckpoint(filepath = f'{model_name}_grooming.h5', verbose=1, save_best_only=True, save_freq='epoch', monitor='val_loss')
            elif BEHAVIOR_NAMES[b_idx] == 'Rearing':
                sav = ModelCheckpoint(filepath=f'{model_name}_rearing.h5', verbose=1,save_best_only=True, save_freq='epoch', monitor='val_loss')
            else:
                sav = ModelCheckpoint(filepath=f'{model_name}.h5', verbose=1, save_best_only=True, save_freq='epoch', monitor='val_loss')

        else:
            model_name = config['model_name']
            if BEHAVIOR_NAMES[b_idx] == 'Grooming':
                sav = ModelCheckpoint(filepath = f'{model_name}_grooming.keras', verbose=1, save_best_only=True, save_freq='epoch', monitor='val_loss')
            elif BEHAVIOR_NAMES[b_idx] == 'Rearing':
                sav = ModelCheckpoint(filepath=f'{model_name}_rearing.keras', verbose=1,save_best_only=True, save_freq='epoch', monitor='val_loss')
            else:
                sav = ModelCheckpoint(filepath=f'{model_name}.keras', verbose=1, save_best_only=True, save_freq='epoch', monitor='val_loss')
        ...
        # Before evaluating load best version of the model
        if config['layers'] == 'tcn':
            with keras.utils.custom_object_scope({'TCN': TCN}):
                if BEHAVIOR_NAMES[b_idx] == 'Grooming':
                    model = keras.models.load_model(f'{model_name}_grooming.h5')
                elif BEHAVIOR_NAMES[b_idx] == 'Rearing':
                    model = keras.models.load_model(f'{model_name}_rearing.h5')
                else:
                    model = keras.models.load_model(f'{model_name}.h5')
        else:
                if BEHAVIOR_NAMES[b_idx] == 'Grooming':
                    model = keras.models.load_model(f'{model_name}_grooming.keras')
                elif BEHAVIOR_NAMES[b_idx] == 'Rearing':
                    model = keras.models.load_model(f'{model_name}_rearing.keras')
                else:
